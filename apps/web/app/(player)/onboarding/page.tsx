  ...
  if (!event) {
    return (
      <main className="mx-auto flex min-h-svh max-w-md flex-col justify-center px-6 text-center">
        <h1 className="text-2xl font-semibold text-white">No active event</h1>
        <p className="mt-2 text-sm text-portage-100">
          Check back when the next estimathon opens.
        </p>
        <Link href="/" className="mt-6 text-sm text-white underline-offset-4 hover:underline">
          ← Back to landing
        </Link>
      </main>
    );
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <div className="mb-8 text-center">
        <p className="text-xs tracking-wide text-portage-100 uppercase">{event.name}</p>
