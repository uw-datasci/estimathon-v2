    ...
    <div className="mx-auto flex min-h-svh max-w-lg flex-col justify-center px-6 py-16">
      <div className="rounded-2xl border bg-card p-8 shadow-xl">
        ...
        {isActive && event?.endsAt ? (
          <div className="mt-10 rounded-xl border bg-muted p-6">
            <p className="text-sm font-medium">{event.name}</p>
            <p className="mt-1 text-sm text-muted-foreground">Event in progress - ends in</p>
            <div className="mt-6 flex justify-center">
              <Countdown target={event.endsAt} />
            </div>
          </div>
        ) : (
          <p className="mt-10 rounded-xl border border-dashed bg-muted p-6 text-center text-sm text-muted-foreground">
